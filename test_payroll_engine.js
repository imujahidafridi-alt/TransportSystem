import { getDb } from './dist/main/database/db.js';
import {
  ...
  getSalaryById,
} from './dist/main/services/salaryService.js';
import { cryptoRandomUUID } from './dist/main/utils/uuid.js';
...
const d1Salary = db.prepare('SELECT * FROM driver_salary_records WHERE driver_id = ? AND salary_period = ?').get(d1Id, TEST_PERIOD);
const d2Salary = db.prepare('SELECT * FROM driver_salary_records WHERE driver_id = ? AND salary_period = ?').get(d2Id, TEST_PERIOD);
...
// Clean up test data
db.prepare('DELETE FROM driver_salary_records WHERE salary_period = ?').run(TEST_PERIOD);
db.prepare('DELETE FROM transports WHERE date >= ? AND date < ?').run('2026-11-01', '2026-12-01');
db.prepare('DELETE FROM drivers WHERE id IN (?, ?, ?)').run(d1Id, d2Id, d3Id);
